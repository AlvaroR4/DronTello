...
class MinimalTelloPublisher(Node):
    def __init__(self):
        super().__init__('tello_image_publisher_minimal')
        self.get_logger().info('Nodo minimal: iniciando...')
        ...
        self._reader.start()
        self.get_logger().info('Iniciado publisher minimal.')

    def _on_video_frame(self, event, sender, data):
        # escribe directamente bytes H264 al stdin de ffmpeg
        try:
            if self.ffmpeg and self.ffmpeg.stdin:
                self.ffmpeg.stdin.write(data)
        except Exception:
            # en versión minimal no reiniciamos; sólo imprimimos y dejamos que falle si es grave
            print('Warning: fallo escribiendo a ffmpeg stdin', file=sys.stderr)
    ...
    def destroy_node(self):
        self.get_logger().info('Cerrando minimal node...')
        self._stop = True
        try:
            if self.tello:
