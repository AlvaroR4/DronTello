...
class NodoDeteccionAruco(Node):
    ...
    def callback_pose(self, msg: Float32MultiArray):
        """ Actualiza la posición y orientación del dron. """
        pose_data = list(msg.data)
        if len(pose_data) >= 6:
            self.pos_dron_mundo = np.array([pose_data[0], pose_data[1], pose_data[2]])
            self.roll = pose_data[3]
            self.pitch = pose_data[4]
            self.yaw = pose_data[5]

    def callback_procesamiento_imagen(self, msg_imagen):
        """ Procesa cada frame de la cámara para detectar el ArUco. """
        try:
        ...
        if ids is not None:
            # 1. Calculos en 2D (imagen)
            # extraer las esquinas y el centro en píxeles
            esquinas = corners[0].reshape((4, 2))
            cx = int(np.mean(esquinas[:, 0]))
            cy = int(np.mean(esquinas[:, 1]))

            # calcular la distancia al marcador
            lado_px = np.linalg.norm(esquinas[0] - esquinas[1])
            distancia = (MARKER_SIZE * FOCAL_PIXELS) / lado_px

            if distancia > 0:
                # 2. Transformación de 2D a 3D (Cuerpo del Dron) 
                punto_cuerpo = self.punto_imagen_a_punto_cuerpo(cx, cy, distancia)

                # 3. Transformación de 3D (Cuerpo) a 3D (Mundo) 
                punto_mundo = self.transformar_punto_cuerpo_a_mundo(punto_cuerpo)

                # 4. Publicar resultados 
                self.publicar_punto_mundo(punto_mundo, msg_imagen.header.stamp)
                self.publicar_punto_y_angulo(punto_mundo, self.yaw)
            
                # Dibujar info
                self.dibujar_debug_info(frame_bgr, esquinas, ids[0], distancia, punto_mundo)
        ...
        try:
            ...
            self.get_logger().error(f"Error al publicar imagen de debug: {e}")

    def punto_imagen_a_punto_cuerpo(self, cx, cy, distancia):
        # Calculamos el punto 3D en el sistema de la cámara (+x derecha, +y abajo, +z adelante)
        z_cam = distancia
    ...
    def transformar_punto_cuerpo_a_mundo(self, punto_cuerpo):
        ...
        # Calculamos la matriz de rotación del cuerpo al mundo (convención ZYX)
        ...
        # Aplicamos la transformación: P_mundo = T_dron_en_mundo + R * P_cuerpo
    ...
    def publicar_punto_mundo(self, punto, stamp):
        ...
        msg.header.frame_id = 'map' # O el frame de referencia que uses
        msg.point.x = punto[0]
        msg.point.y = punto[1]
        msg.point.z = punto[2]
    ...
    def dibujar_debug_info(self, imagen, esquinas, id_marcador, dist, punto_mundo):
        cv2.polylines(imagen, [esquinas.astype(np.int32)], True, (0, 255, 0), 2)
        
        texto_id = f"ID: {id_marcador[0]}"
        texto_dist = f"Dist: {dist:.2f} m"
        texto_mundo = f"Mundo: ({punto_mundo[0]:.2f}, {punto_mundo[1]:.2f}, {punto_mundo[2]:.2f})"

        esquina_superior_izq = tuple(esquinas[0].astype(int))
        cv2.putText(imagen, texto_id, (esquina_superior_izq[0], esquina_superior_izq[1] - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
        cv2.putText(imagen, texto_dist, (esquina_superior_izq[0], esquina_superior_izq[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
...
def main(args=None):
    ...
    try:
        ...
        print("Cerrando nodo por interrupción del teclado.")
    finally:
